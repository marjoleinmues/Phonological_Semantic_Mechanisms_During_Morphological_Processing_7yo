...
function firstlevel_sem_ses7 (subjects)
parpool;
...
analysis_folder='ses7_analysis_sem'; % the name of your first level modeling folder 
...
CCN.func_pattern='sub*Sem*_bold'; % the name of your functional folders
CCN.file='vs6_wsub*Sem*_bold.nii'; % the name of your preprocessed data (4d)%
CCN.rpfile='rp_*Sem*.txt'; %the movement files
...
conditions=[];
conditions{1}={'S_L' 'S_H' 'S_U' 'S_C'}; %Run1
conditions{2}={'S_L' 'S_H' 'S_U' 'S_C'}; %Run2
...
%matched.
contrasts={'HighLow_Cont'};
HighLow_Cont=[1 1 0 -2];


%adjust the contrast by adding six 0s into the end of each session
rp_w=zeros(1,6);
% empty=zeros(1,10);
weights={[HighLow_Cont rp_w HighLow_Cont rp_w]};
