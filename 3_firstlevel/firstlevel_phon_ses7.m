...
% The repaired images will be deweighted from 1 to 0.01 in the first levelregressors to remove the small motion effects on data.
...
function firstlevel_phon_ses7 (subjects)
parpool;
...
analysis_folder='ses7_analysis_phon'; % the name of your first level modeling folder 
...
CCN.func_pattern='sub*Phon*_bold'; % the name of your functional folders
CCN.file='vs6_wsub*Phon*_bold.nii'; % the name of your preprocessed data (4d)%
CCN.rpfile='rp_*Phon*.txt'; %the movement files
...
conditions=[];
conditions{1}={'P_R' 'P_O' 'P_U' 'P_C'}; %Run1
conditions{2}={'P_R' 'P_O' 'P_U' 'P_C'}; %Run2
...
%matched.
contrasts={'RhymeOnset_Cont'};
RhymeOnset_Cont=[1 1 0 -2];


%adjust the contrast by adding six 0s into the end of each session
rp_w=zeros(1,6);
% empty=zeros(1,10);
weights={[RhymeOnset_Cont rp_w RhymeOnset_Cont rp_w]};
